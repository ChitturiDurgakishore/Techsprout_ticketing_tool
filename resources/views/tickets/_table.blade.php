    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Status</th>
                <th>Priority</th>
                @if(isset($extraCol) && $extraCol === 'created_by')
                    <th>Created By</th>
                @elseif(isset($extraCol) && $extraCol === 'assigned_to')
                    <th>Assigned To</th>
                @endif
                <th>Date</th>
            </tr>
        ...
        <tbody>
            @foreach($tickets as $ticket)
                ...
                <tr onclick="window.location.href='{{ route('tickets.show', $ticket) }}'" class="{{ $rowClass }}" style="cursor: pointer;">
                    ...
                    <td>
                        <div class="text-sm font-semibold" style="color: var(--accent);">{{ $ticket->project->name ?? '—' }}</div>
                    ...
                    @if(isset($extraCol) && $extraCol === 'created_by')
                        <td class="text-sm">{{ $ticket->createdBy->name ?? '—' }}</td>
                    @elseif(isset($extraCol) && $extraCol === 'assigned_to')
                        <td>
                            <div class="flex items-center gap-2">
